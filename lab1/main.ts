...
type TAngle = 'leg' | 'hypotenuse'| 'adjacent angle' | 'opposite angle' | 'angle';

function triangle(val1: number, type1: TAngle, val2: number, type2: TAngle): string {
    const validTypes: TAngle[] = ['leg', 'hypotenuse', 'adjacent angle', 'opposite angle', 'angle'];
    const angleTypes: TAngle[] = ['adjacent angle', 'opposite angle', 'angle'];
    ...
    // Value Validations
    const value1: number = Number(val1);
    const value2: number = Number(val2);
    ...
    const checkAngle = (type: TAngle, value: number): boolean => {
    ...
    if ((type1 === 'hypotenuse' && type2 === 'leg') || (type2 === 'hypotenuse' && type1 === 'leg')) {
        const hypotenuse: number = type1 === 'hypotenuse' ? value1 : value2;
    ...
    const processType = (type: TAngle, value: number): void => {
        switch (type) {
            case 'leg':
